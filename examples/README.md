...
| [`05_mixture.py`](05_mixture.py) | A Gaussian mixture with the cluster labels **sampled**, not marginalized: `int<lower=1,upper=k>` parameters and the Metropolis-within-Gibbs sweep the factory composes over NUTS for you. Why the means are `ordered`, and why the label-move count is the diagnostic to read first. |
