...
- **Extra checks at every merge** (`extra_uturn_checks`, on by default; Stan since 2.21, 2019). Merging $L = [a..m]$ with $R = [m+1..b]$ tests the criterion over $[a..b]$ and also over $[a..m+1]$ and $[m..b]$: each half extended by the adjacent leaf of the other. Without them a U-turn that straddles the boundary between two subtrees is invisible to every check, and on a near-isotropic target with a well-matched mass the trajectory can loop around the mode to the maximum depth — mean tree depth 6.3–6.9 against 2.8 on a unit-mass Gaussian at $d = 20$, $\epsilon = 0.78$. They apply inside subtrees (a failure invalidates the subtree) and at the top-level merge in `_build_nuts` (a failure stops expansion, keeping the subtree). At a merge of two single leaves both spans are the whole block, so they start at level 2. Each check is a function of a canonical block's two halves, so reversibility is unaffected; they only add checks, so a trajectory can only get shorter. `extra_uturn_checks=False` restores the original rule bit for bit.
