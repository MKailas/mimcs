...
That hazard's *correctness* form is why `parallel_tempering` refused a model with discrete parameters until doc 14's work. It no longer does, and what it took is worth recording, because the refusal's own comment named only two of the three obstacles.

**`_lanes` and `ProductKinetic.precompute` build their per-lane contexts positionally**, so `discrete` was dropped there exactly as `betas` was. Both now forward every field by keyword, and `_lanes` **slices** `discrete` per lane rather than forwarding the whole product block — a lane's context is supposed to describe one temperature, and handing it the whole ladder's labels would be a landmine for any future kinetic that read it.

**`TemperedProductPotential` vmapped over lanes with the *shared* context** — `jax.vmap(lambda qk: self.inner.potential(qk, ctx))`. This was not in the refusal comment and is the site that actually decides correctness: every rung would have evaluated its density at the same labels. All four entry points now route through one `_lane_potentials`, which zips each lane's own row in. The continuous path is kept as *literally* the old expression, so a tempered run without discrete parameters emits the graph it always did.

**And `_swap` permuted only `coordinate`.** A replica is its whole state: exchanging positions while leaving the labels behind hands each rung a configuration drawn from no target at all, and nothing downstream would show it. `apply_swaps` was already generic over the trailing shape of a `(K, …)` array, so the fix is one extra call — the danger was entirely in not noticing. Two ordering details go with it: the swap *ratio* is computed before the permutation (it compares each rung's current state, labels included), and the potential caches are re-seeded *after* it against the **post-swap** labels, since the context in hand was built from the pre-swap state.

The proposal tables are the exception that proves the rule: `discrete_proposal_params` is deliberately **not** permuted. A table describes a *temperature*, not a state — after a swap rung `k` holds a different replica but still targets `pi^beta_k`, so its table still approximates the right marginal.
