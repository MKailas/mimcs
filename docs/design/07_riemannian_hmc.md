...
serves, non-finite units skipped, the Polyak average frozen for sampling), and `A` by feeding the
...
`A` is whitened by the *raw* `D(x)` iterate, which also drives warmup, while a uniform Polyak mean
of `D(x)` is frozen for sampling. `metric_ema_warmup` (experimental, off) instead keeps an
exponential moving average of the `D(x)` parameters with the SGD's Robbins–Monro gain, as
`ScoreMassAdaptation` smooths its mass. The EMA drives the simulation, whitens `A` and is frozen,
so the shape is fitted under the metric the chain actually runs with, and adapts more slowly. It
is `MetricAdaptation`'s key, so plain and shaped blocks run the same `D(x)`. A position-*varying* shape `A(x)` is a deliberate non-goal for now. Settable by
