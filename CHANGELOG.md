...
- **Learned metrics can condition on discrete parameters.** A metric is fitted to the conditional
  ...
  the fitted quantity is `E[g g' | q, z]` rather than its average over `z`. That matters because
  labels change a mode's *scale* --- under a spike-and-slab indicator `beta_j` has scale `tau` or
  near zero, and one mass is wrong under both. A dependency is **declared**, not inferred:
  `Exp("sigma", categorical=["z"], ordinal=["k"])`. An ordinal label is standardized by its
  **declared support** rather than by the observed draws, because the transform must be defined
  with no evidence at all (`MetricAdaptation` starts cold, and a hand-written metric never sees a
  pilot); a categorical one is **reference-coded** into `k - 1` indicators, since full one-hot
  duplicates the additive `+ Exp()` and leaves a direction the optimizer cannot resolve but AIC
  still charges for. At `k = 2` the two coincide, so only one is offered. Selection is two-pass:
  the existing pool plus discrete-only forms, then the best *continuous* candidate multiplied by
  each discrete factor. Against a target whose ideal metric is known in closed form, the offline
  fit recovers `W = -2.312` against `-2.3026`, the online SGD `-1.617` against `-1.609`, and the
  sampler reproduces both conditional scales (0.992, 0.206 against 1.0, 0.2) at zero divergences;
  a target where the labels do **not** affect the scale keeps the constant baseline, the control
  that decides whether the second pass adopts noise. Under tempering each rung learns the metric
  at its own labels, checked structurally rather than by tolerance --- tempering `N(0, s^2)` gives
  `M = beta/s^2`, so the contrast is temperature-invariant while the level shifts by `log beta`,
  measured flat at [1.461, 1.508, 1.614] over three rungs against a cold-rung truth of 1.609.
  Continuous models are bit-identical. The product-of-sums target form already worked and needed
  no change to the algebra; the gap that did need one was `AdaptState`, which had no `discrete`
  field, so a tempered learned metric would have adapted against no labels at all.
